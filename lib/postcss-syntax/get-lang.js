...
function sourceType(source) {
  ...
  if (
    ...
    // eslint-disable-next-line regexp/no-super-linear-backtracking, regexp/optimal-quantifier-concatenation, regexp/no-unused-capturing-group, regexp/no-useless-lazy
    /^(?:(?:\/\/[^\r\n]*\r?\n|\/\*.*?\*\/)\s*)*(?:(?:("|')use strict\1|import(?:\s+[^;]+\s+from)?\s+("|')[^'"]+?\2|export\s+[^;]+\s+[^;]+)\s*(;|\r?\n|$)|(?:(?:var|let|const)\s+[^;]+\s*=\s*)?(?:require|import)\(.+\))/.test(
      source
    ) ||
...
function fileType(file) {
  // eslint-disable-next-line regexp/no-unused-capturing-group, regexp/no-useless-lazy
  if (file && /\.(\w+)(?:[?#].*?)?$/.test(file)) {
    // eslint-disable-next-line regexp/no-legacy-features
